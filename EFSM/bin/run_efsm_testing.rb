...
def cleanup()
	...
	execute_shell_cmd("for pid in $(ps a | grep 'efsm_test.jar' | awk '{print $1}'); do kill -9 $pid; done")
...
def construct_efsm(app_dir)
	execute_shell_cmd("python android_efsm/modelConstruction.py #{app_dir}")
end
...
def construct_fsm(app_dir, apk_path, avd_serial="emulator-5554", stoat_port="2000")
	...
	# clsed-source projects
		...
		Dir.chdir($STOAT_TOOL_DIR) do
			job2 = fork do 
				cmd = "bash -x ./bin/EFSManalyzeAndroidApk.sh fsm_apk #{app_dir} apk #{apk_path} &> /dev/null &"
				puts "$ #{cmd}"
		  		exec cmd
			end
			Process.detach(job2)
	...
	# open-source projects
		...
		Dir.chdir(app_dir) do
			...
			Dir.chdir($STOAT_TOOL_DIR) do
				job2 = fork do 
					cmd = "bash -x ./bin/EFSManalyzeAndroidApk.sh fsm #{app_dir_path} #{$project_type} #{apk_path} &> /dev/null &"
					puts "$ #{cmd}"
			  		exec cmd
				end
				Process.detach(job2)
...
def test_generate_execute_open_source_projects(app_dir, apk_path, avd_serial, stoat_port)
	...
	Dir.chdir(app_dir) do
		...
		if not File.exist?(fsm_file) then
			...
			if not (fsm_states >= 2) then
				...
				Dir.chdir($STOAT_TOOL_DIR) do
					job2 = fork do 
				  		exec "./bin/EFSManalyzeAndroidApk.sh mcmc #{app_dir_path} #{$project_type} #{apk_path}&> /dev/null &"
					end
					Process.detach(job2)
				end
...
def test_generate_execute_closed_source_projects(app_dir, apk_path, avd_serial, stoat_port)
	...
	Dir.chdir(app_dir_loc) do
		...
		if not File.exist?(fsm_file) then
			...
			if not (fsm_states >= 2) then
				...
				Dir.chdir($STOAT_TOOL_DIR) do
					job2 = fork do 
				  		exec "./bin/EFSManalyzeAndroidApk.sh mcmc_apk #{app_dir} #{$project_type} #{apk_path}&> /dev/null &"
					end
					Process.detach(job2)
				end
...
def test_generate_execute(app_dir, apk_path, avd_serial="emulator-5554", stoat_port="2000")


	if app_dir.end_with?(".apk") then
		test_generate_execute_closed_source_projects(app_dir, apk_path, avd_serial, stoat_port)
	else
		test_generate_execute_open_source_projects(app_dir, apk_path, avd_serial, stoat_port)
	end
...
if (not app_dir.eql?("")) && File.exist?(app_dir) then  # for testing one app at one time
	...
	if $only_gui_exploration then
		...
		construct_fsm(app_dir, apk_path, avd_serial, stoat_port)
		construct_efsm(app_dir)
		test_generate_execute(app_dir, apk_path, avd_serial, stoat_port)
	end
	...
	File.readlines(apps_list).each do |line|
		...
		if $only_gui_exploration then
			...
			construct_fsm(app_dir_from_file, apk_path_from_file, avd_serial, stoat_port)
			construct_efsm(app_dir)
			test_generate_execute(app_dir_from_file, apk_path_from_file, avd_serial, stoat_port)
		end
