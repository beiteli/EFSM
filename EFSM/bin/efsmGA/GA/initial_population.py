...
def get_startlist():
    ...
    startlist = []
    for line in range(len(traninfolist)):
        if traninfolist[line].src.name == "S_1":
    ...
    while 1:
        while 1:
            ...
            if (m[i][randnum] == 1):
                path.append(randnum)
                counter += 1
                # print counter, randnum
                break
        if counter >= min(obtain_efsm_info.tran_number(), obtain_efsm_info.state_number()):
            break
...
def random_subpath(transition):  # 以给定transition为开始的子path
    ...
    path.append(i)
    count = 0
    while count < random.randint(2, 8):
        while 1:
            randnum = random.randint(0, len(m[i]) - 1)
            if (m[i][randnum] == 1):
                path.append(randnum)
