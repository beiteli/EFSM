...
class Agent
    ...
    def execute_event(action_cmd, action_view_text, pkg)
...
#        puts "pkg:#{pkg}"
        ...
        if action_type.eql?("edit") && action_param.eql?("_by_resource_id") then
          ...
          #puts "[D]: #{action_type}, #{action_param}, #{action_param_value}"
           # construct the python cmd
           event_cmd = "timeout 2s python ./bin/events/agent_#{action_type}#{action_param}.py #{@emulator_serial} #{action_param_value} #{action_view_text}"
           `#{event_cmd}`
           puts "$ #{event_cmd}"
        elsif action_type.eql?("clickbutton") && action_param.eql?("_by_resource_id") then
          first_quote_index = action_cmd.index("\'") # get the action param value
          last_quote_index = action_cmd.rindex("\'")
          # Note we should include the quotes to avoid the existence of whitespaces in the action_param_value
          action_param_value = action_cmd[first_quote_index..last_quote_index]
          #puts "[D]: #{action_type}, #{action_param}, #{action_param_value}"
            # construct the python cmd
            event_cmd = "timeout 2s python ./bin/events/agent_click_button#{action_param}.py #{@emulator_serial} #{action_param_value} #{action_view_text}"
            `#{event_cmd}`
            puts "$ #{event_cmd}"
        elsif action_type.eql?("clickbutton") && action_param.eql?("_by_classname_instance") then
          ...
          compare_pattr = /@=\'(.*)\'/
          compare = (action_cmd.match compare_pattr)[1]
            # construct the python cmd
          event_cmd = "timeout 2s python ./bin/events/agent_click_button_by_classname_instance.py #{@emulator_serial} #{class_name} #{instance} #{class_name}"
          `#{event_cmd}`
          puts "$ #{event_cmd}"
        elsif action_param.eql?("_by_classname_instance") then
          # execute action by classname and instance
          class_name_pattr = /className=\'(.*)\',/
          class_name = (action_cmd.match class_name_pattr)[1]
          instance_pattr = /instance=\'(.*)\'/
	...
	#  UTIL.login(@emulator_serial, "wechat")
        ...
        # end
        inject_pos = Random.new.rand(test_sequence_length)
        inject_posbroadcast = Random.new.rand(test_sequence_length)
        inject_poslistener = Random.new.rand(test_sequence_length)
        inject_poslauncher = Random.new.rand(test_sequence_length)
        inject_posservice = Random.new.rand(test_sequence_length)
        ...
        for t in test_sequence
            ...
            if inject_pos > test_sequence_length/2 and inject_pos > inject_posbroadcast and i == inject_pos then
            ...
            # if i == inject_posbroadcast then
            #   # inject the system event
            #   system_event = "python #{@stoat_tool_dir}/trigger/tester.py -s #{@emulator_serial} -f #{@apk} -p broadcast"
            #   system_event_output = UTIL.execute_shell_cmd(system_event)
            #   puts "**********#{system_event_output}**********"
            #   sleep (@event_delay) # wait seconds
            #   @crash_reporter.log_test_execution_info(system_event_output, "", i, true)
            # end
            # if i == inject_poslistener then
            #   # inject the system event
            #   system_event = "python #{@stoat_tool_dir}/trigger/tester.py -s #{@emulator_serial} -f #{@apk} -p listener"
            #   system_event_output = UTIL.execute_shell_cmd(system_event)
            #   puts "**********#{system_event_output}**********"
            #   sleep (@event_delay) # wait seconds
            #   @crash_reporter.log_test_execution_info(system_event_output, "", i, true)
            # end
            # if inject_pos > test_sequence_length/2 and i == inject_poslauncher then
            #   # inject the system event
            #   system_event = "python #{@stoat_tool_dir}/trigger/tester.py -s #{@emulator_serial} -f #{@apk} -p launcher"
            #   system_event_output = UTIL.execute_shell_cmd(system_event)
            #   puts "**********#{system_event_output}**********"
            #   sleep (@event_delay) # wait seconds
            #   @crash_reporter.log_test_execution_info(system_event_output, "", i, true)
            # end
            # if i == inject_posservice then
            #   # inject the system event
            #   system_event = "python #{@stoat_tool_dir}/trigger/tester.py -s #{@emulator_serial} -f #{@apk} -p service"
            #   system_event_output = UTIL.execute_shell_cmd(system_event)
            #   puts "**********#{system_event_output}**********"
            #   sleep (@event_delay) # wait seconds
            #   @crash_reporter.log_test_execution_info(system_event_output, "", i, true)
            ...
            res = execute_event(action_cmd, action_view_text, pkg)
            sleep (@event_delay) # wait seconds
