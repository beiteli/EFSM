 ...
 * ------------------------ */
   ...
   */
   public LUDecomposition (double[][] A) {       //Matrix A) {
 ...
 * ------------------------ */
   ...
   */
   public double[][] getL () {
      ...
      double[][] L = new double[m][n];  //X.getArray();
      for (int i = 0; i < m; i++) {
         for (int j = 0; j < n; j++) {
            if (i > j) {
               L[i][j] = LU[i][j];
   ...
   */
   public double[][] getU () {
      ...
      double[][] U = new double[n][n];   //X.getArray();
      for (int i = 0; i < n; i++) {
         for (int j = 0; j < n; j++) {
            if (i <= j) {
               U[i][j] = LU[i][j];
   ...
   */
   public double[][] solve (double[][] B) {
      ...
      int nx = B[0].length;    //.getColumnDimension();
      //Matrix Xmat = B.getMatrix(piv,0,nx-1);

      // get matrix
      double[][] X = new double[B.length][nx]; //piv.length][nx];
      //double[][] B = X.getArray();
      try {
         ...
         throw new ArrayIndexOutOfBoundsException("LUDecomposition Submatrix indices");
      }
      //return X;
